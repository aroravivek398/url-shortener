...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/public/register`, {
        method: 'POST',
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-panel rounded-xl border border-panel-2 overflow-hidden">
          <div className="flex items-center gap-1.5 px-4 py-3 border-b border-panel-2">
            ...
            <span className="ml-2 text-xs font-mono text-muted">register</span>
          </div>

          <form onSubmit={handleSubmit} className="p-6">
            <h1 className="font-display font-bold text-xl text-paper mb-1">Create account</h1>
            <p className="text-sm text-muted mb-6">Start shortening links in seconds</p>
            ...
            <p className="text-sm text-muted mt-6 text-center">
              ...
              <Link to="/login" className="text-amber hover:underline">Log in</Link>
            </p>
          </form>
        </div>
      </div>
